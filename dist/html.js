...
function collectViolations(collected) {
    const entries = [];
    for (const item of collected) {
        if (!item.axeResults?.violations)
            continue;
        const pageUrl = item.url;
        for (const v of item.axeResults.violations) {
            entries.push({ pageUrl, violation: v });
        }
    }
    return entries;
}
function collectIncomplete(collected) {
    const entries = [];
    for (const item of collected) {
        if (!item.axeResults?.incomplete)
            continue;
        const pageUrl = item.url;
        for (const v of item.axeResults.incomplete) {
            entries.push({ pageUrl, violation: v });
        }
    }
    return entries;
}
function collectPasses(collected) {
    const entries = [];
    for (const item of collected) {
        if (!item.axeResults?.passes)
            continue;
        const pageUrl = item.url;
        for (const v of item.axeResults.passes) {
            entries.push({ pageUrl, violation: v });
        }
    }
    return entries;
}
function collectInapplicable(collected) {
    const entries = [];
    for (const item of collected) {
        if (!item.axeResults?.inapplicable)
            continue;
        const pageUrl = item.url;
        for (const v of item.axeResults.inapplicable) {
            entries.push({ pageUrl, violation: v });
        }
    }
    return entries;
}
function collectAxeVersion(collected) {
    for (const item of collected) {
...
function renderRow(pageUrl, v, index) {
...
function renderIncompleteRow(pageUrl, v, index) {
    const nodeCards = v.nodes.map((node, ni) => renderNodeCard(node, ni)).join("");
    return `
          <details>
            <summary>
...
function renderPassRow(pageUrl, v, index) {
    const nodeCards = v.nodes.map((node, ni) => renderNodeCard(node, ni)).join("");
...
function renderInapplicableRow(pageUrl, v, index) {
    return `
          <details>
            <summary>
              <div aria-hidden="true" class="toggle-cell">
...
function buildHtml(violationRows, incompleteRows, passRows, inapplicableRows, violationCount, incompleteCount, passCount, inapplicableCount, showIncomplete, showPasses, showInapplicable, meta) {
...
export function generateHtmlReport(tmpDir, config, cwd = process.cwd()) {
    ...
    const html = buildHtml(violationRows, incompleteRows, passRows, inapplicableRows, violations.length, incomplete.length, passes.length, inapplicable.length, config.showIncomplete, config.showPasses, config.showInapplicable, {
