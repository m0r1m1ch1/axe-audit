...
import { AuditError, DEFAULT_CONFIG, VALID_LOCALES, } from "./types.js";
...
export function mergeCliOptions(config, cli) {
    return {
        ...config,
...
export function validateConfig(userConfig) {
    ...
    if (userConfig.axe !== undefined) {
        ...
        if (ua.tags !== undefined) {
            if (!Array.isArray(ua.tags) || !ua.tags.every((t) => typeof t === "string")) {
        ...
        if ("locale" in ua) {
            if (ua.locale === undefined) {
                ...
                if (!VALID_LOCALES.includes(ua.locale)) {
                    throw new AuditError(`Invalid locale "${ua.locale}". Valid locales: ${VALID_LOCALES.join(", ")}`);
                }
        ...
        if (ua.disableRules !== undefined) {
            if (!Array.isArray(ua.disableRules) || !ua.disableRules.every((t) => typeof t === "string")) {
        ...
        if (ua.include !== undefined) {
            if (!Array.isArray(ua.include) || !ua.include.every((t) => typeof t === "string")) {
                throw new AuditError("config 'axe.include' must be an array of strings.");
            }
            axe.include = ua.include;
        }
        if (ua.exclude !== undefined) {
            if (!Array.isArray(ua.exclude) || !ua.exclude.every((t) => typeof t === "string")) {
