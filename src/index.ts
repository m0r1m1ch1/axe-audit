...
import { type CliOptions, AuditError } from "./types.js";
...
function parseCli(): CliOptions {
  ...
  return {
    noBuild: values["no-build"] as boolean,
...
function handleInit(cwd: string): void {
...
function serveAndOpen(htmlPath: string): Promise<void> {
...
async function main(): Promise<void> {
  ...
  if (existsSync(resultsPath)) {
    try {
      const raw = JSON.parse(readFileSync(resultsPath, "utf-8")) as Array<{
        axeResults?: { violations?: unknown[] } | null;
      }>;
      hasViolations = raw.some(
        (entry) => (entry.axeResults?.violations?.length ?? 0) > 0
      );
    } catch {
