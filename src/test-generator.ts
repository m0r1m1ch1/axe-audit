...
import { type PageInfo, type ResolvedConfig, AuditError } from "./types.js";
...
function generateResultsReporter(): string {
  return `
import type {
  Reporter,
  TestCase,
...
`;
  ...
  if (axeConfig.tags) {
    ...
    if (axeConfig.aaa) tags.push("wcag2aaa");
    if (axeConfig.experimental) tags.push("experimental");
    tagsCode = `const tags: string[] | undefined = ${JSON.stringify(tags)};`;
  } else {
    const extraTags: string[] = [];
    if (axeConfig.aaa) extraTags.push("wcag2aaa");
    if (axeConfig.experimental) extraTags.push("experimental");
